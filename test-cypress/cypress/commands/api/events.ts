...
Cypress.Commands.add(
  ...
  (user: string, camera: string, expected: boolean) => {
    logTestDescription(
      `Check that ${camera} is ${expected ? "stopped" : "running"}`,
      {
        user,
        camera,
        expected
...
function checkDeviceStopped(user: string, camera: string, expected: boolean) {
  const params = {
...
function checkResponseMatches(response: Cypress.Response, expected: boolean) {
