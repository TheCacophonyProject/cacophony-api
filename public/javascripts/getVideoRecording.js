/**
 * Does an API get request for an Video recording using the value in the userQuery element.
 */
function query() {
  console.log("Quering with:");
  var q = document.getElementById('query-input').value;
  $.ajax({
    url: '/api/v1/videoRecordings\?q\='+encodeURI(q),
    type: 'GET',
    success: queryDone,
    error: queryError
  });
...
function queryDone(fullRes) {
  ...
  for (var res in fullRes) {
    ...
    // Parsing through Video Recording fields.
    for (var field in vrFields) {
      if (fullRes[res][field]) {

        var index;  // Column index
        var cell;     // Cell element
        if (vrFields[field].isChildModel) {
          var arCmFields = vrFields[field];
          ...
          for (cmField in arCmFields) {
            if (cmField != "isChildModel") {
              ...
              if (!vrFields[field][cmField].show) {
                $(cell).hide();
              }
            }
          }
...
function toggleColumn(ele) {
  ...
  if (ele.isChildField) { //Methods of getting the current state differ if is is a child fied.
    show = !vrFields[ele.field][ele.childField].show; //Oposite of what is currently is
    vrFields[ele.field][ele.childField].show = show;
  } else {
    show = !vrFields[ele.field].show; //Oposite of what it currently is.
    vrFields[ele.field].show = show;
  }
 ...
 * Makes a column from each field in the vrFields variable.
 * Will
 *
 */
function generateTableColumns() {
  ...
  for (var field in vrFields) {
    ...
    if (vrFields[field].isChildModel) {
      var childModelFields = vrFields[field];
    ...
    // Add new column for field and saving index of column.
      ...
      vrFields[field].columnIndex = appendColumn(field);
      if (vrFields[field].show) {
        aElement.innerHTML = "✔"+field;
      } else {
        aElement.innerHTML = field;
      }
      showHideColumn(vrFields[field].columnIndex, vrFields[field].show);
      aElement.onclick = toggleColumn;
      aElement.columnIndex = vrFields[field].columnIndex;
...
// JSON of the fields in the video recording.
