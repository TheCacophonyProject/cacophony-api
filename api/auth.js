...
const authenticate = function(type) {
  return async (req, res, next) => {
    ...
    if (type && type != jwtDecoded._type) {
      return res.status(401).json({
        "messages": [format('Invalid type of JWT. Need one of %s for this request, but had %s.', type, jwtDecoded._type)]
      });
    }
    let result;
...
const authenticateAdmin = async (req, res, next) => {
  ...
  if (jwtDecoded._type != 'user') {
    return res.status(403).json({
      "messages": ['Admin has to be a user']
    });
  }
  const user = await models.User.findByPk(jwtDecoded.id);
  if (!user) {
    return res.status(401).json({
      "messages": ['Could not find user from JWT.']
    });
  }
...
const signedUrl = (req, res, next) => {
  ...
  if (jwtParam == null) {
    return res.status(401).json({
      "messages": ['Could not find JWT token in query params.']
    });
  }
  let jwtDecoded;
  try {
