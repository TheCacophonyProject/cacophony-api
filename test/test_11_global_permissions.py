...
class TestGlobalPermission:
    ...
    def test_global_permission_on_recording(self, helper):
        ...
        with pytest.raises(OSError, message="Fred shoudl not be able to delete the recording with only global read permission"):
            fred.delete_recording(recording)

        print("  Or update the recording")
        with pytest.raises(OSError, message="Fred shoudl not be able to update the recording with only global read permission"):
            fred.update_recording(recording, comment='testing')

        print("  Or tag the recording")
        with pytest.raises(OSError, message="Fred shoudl not be able to tag the recording with only global read permission"):
