...
class TestRecordingsGetByTags:
    def testFindRecordingTags(self, helper):
        ...
        human_cool_cat = makeTaggedRecording(device, helper, [], lucy, ["cat", "cool"])
        human_missed_track = makeTaggedRecording(
            device, helper, [], lucy, ["missed track"]
        )
        human_multiple_animals = makeTaggedRecording(
            device, helper, [], lucy, ["multiple animals"]
        )
        human_trapped = makeTaggedRecording(
            device, helper, [], lucy, ["trapped in trap"]
        )

        ai_multiple = makeTaggedRecording(
            device, helper, ["multiple animals"], lucy, []
        )
        ai_possum = makeTaggedRecording(device, helper, ["possum"], lucy, [])
        human_possum = makeTaggedRecording(device, helper, [], lucy, ["possum"])
        ai_human_possum = makeTaggedRecording(
            device, helper, ["possum"], lucy, ["possum", "cool"]
        )

        ai_possum2 = makeTrackTaggedRecording(
            device, helper, lucy, [["possum", None], ["cat", None]]
        )
        human_cat_possum2 = makeTrackTaggedRecording(
            ...
            lucy,
            [["cat", None], ["cat", "possum"]],
            ai_RecordingTags=["multiple animals"],
        )
        ai_human_possum2 = makeTrackTaggedRecording(
            device, helper, lucy, [["possum", "possum"]]
        )

        # ai and human tags different - one track, on recording.
        ai_human_possum3 = makeTrackTaggedRecording(
            device, helper, lucy, [[None, "possum"]]
        )
        ...
        lucy.when_searching_for_tagmode_and_tags(
            "any", []
        ).can_see_all_recordings_from_(all)
        ...
        lucy.when_searching_for().tagmode("tagged").tags(
            ["rat", "cat"]
        ...
        # no animal tags
        expected = [untagged]
        lucy.when_searching_for_tagmode("untagged").can_only_see_recordings(
            *expected
        ).from_(all)

        expected = [untagged, ai_possum, ai_possum2, ai_multiple]
        lucy.when_searching_for_tagmode("no-human").can_only_see_recordings(
            *expected
        ).from_(all)

        expected = [ai_possum, ai_possum2, ai_multiple]
        lucy.when_searching_for_tagmode("automatic-only").can_only_see_recordings(
            *expected
        ).from_(all)
        ...
        lucy.when_searching_for_tagmode("human-only").can_only_see_recordings(
            *expected
        ).from_(all)
        ...
        lucy.when_searching_for_tagmode("automatic+human").can_only_see_recordings(
            *expected
        ).from_(all)

        expected = [human_missed_track]
        lucy.when_searching_for_tagmode("missed track").can_only_see_recordings(
            *expected
        ).from_(all)

        expected = [human_multiple_animals, human_cat_possum2, ai_multiple]
        lucy.when_searching_for_tagmode("multiple animals").can_only_see_recordings(
            *expected
        ).from_(all)

        expected = [human_trapped]
        lucy.when_searching_for_tagmode("trapped in trap").can_only_see_recordings(
            *expected
        ).from_(all)

        expected = [ai_human_possum, human_cool_cat]
        lucy.when_searching_for_tagmode("cool").can_only_see_recordings(
            *expected
        ).from_(all)

        expected = [human_cool_cat]
        lucy.when_searching_for_tagmode("cool").tags(["cat"]).can_only_see_recordings(
            *expected
        ).from_(all)
    ...
    def testInterestingRecordingTags(self, helper):
        ...
        false_positive = makeTaggedRecording(
            device, helper, ["false positive"], julie, []
        )
        bird = makeTaggedRecording(device, helper, [], julie, ["bird"])

        all = [animal, false_positive, bird]

        julie.when_searching_for_tagmode_and_tags(
            "tagged", ["interesting"]
    ...
    def testInterestingRecordingTrackTags(self, helper):
        ...
        false_positive = makeTrackTaggedRecording(
            device, helper, julie, [[None, "false positive"]]
        )
...
def makeTrackTaggedRecording(
    ...
    recording = makeTaggedRecording(
        device, helper, ai_RecordingTags, tagger, human_recordingTags
    )
