...
class TestStations:
    def test_add_stations(self, helper):
        ...
        added_station_ids = station_user.add_stations_to_group(station_group['groupId'], json.dumps([{
            "name": "Station name",
            "lat": -43.62367659982135,
            "lng": 172.62646754804894
        }]))
        ...
        with pytest.raises(BadRequestError) as error:
            station_user.add_stations_to_group(station_group['groupId'], json.dumps([{
                ...
                "name": "Too close station",
                "lat": -43.62367657911904,
                "lng": 172.62626365029485
            }]))
        ...
        with pytest.raises(AuthorizationError) as error:
            station_user.add_stations_to_group(other_station_group['groupId'], json.dumps([{
                "name": "Station name 2",
                "lat": -43.5338773,
                "lng": 172.6451473
            }]))
        ...
        with pytest.raises(UnprocessableError) as error:
            station_user.add_stations_to_group(99999, json.dumps([{
                "name": "Station name 2",
                "lat": -43.5338773,
                "lng": 172.6451473
            }]))
        ...
        current_stations = station_user.get_stations_for_group(station_group_name)
        retired = [
            x for x in current_stations['stations']
    ...
    def test_adding_recordings_matched_to_stations(self, helper):
        ...
        created_recording = helper.given_a_recording(self, group=station_group_name, props={
            "location": [-43.623704659672676, 172.6267505162639]
        })
        assert created_recording

        print("Get the data for the uploaded recording, and see which station (if any) it has been matched to")
    ...
    def test_backdating_new_stations_to_existing_recordings(self, helper):
        ...
        one_minute_ago = datetime.fromtimestamp(datetime.now().timestamp() - 60, dateutil.tz.gettz("Pacific/Auckland"))
        station_user.add_stations_to_group(station_group['groupId'], json.dumps([{
            "name": "Station name",
            "lat": -43.62367659982135,
            "lng": 172.62646754804894
